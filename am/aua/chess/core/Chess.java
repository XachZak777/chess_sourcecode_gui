...
public class Chess implements Cloneable {
    ...
    public boolean isWithinBounds(Position p) {
        return p.getRank() >= 0 && p.getRank() < BOARD_RANKS &&
               p.getFile() >= 0 && p.getFile() < BOARD_FILES;
    }
    ...
    public boolean isValidMove(Move move) {
        ...
        if (!isWithinBounds(origin) || !isWithinBounds(destination))
            return false;
        ...
        if (piece.getPieceColor() != getTurn())
            return false;

        Position[] reachablePositions = reachableFrom(origin);
        for (Position pos : reachablePositions) {
            if (pos.equals(destination)) {
                Chess clone = this.clone();
                clone.performMove(move);
                if (clone.isKingUnderAttack(getTurn()))
                    return false;
                return true;
            }
        }
